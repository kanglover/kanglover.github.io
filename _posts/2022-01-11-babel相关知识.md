...
* 当你使用generators/async函数时，自动引入@babel/runtime/regenerator（可通过regenerator选项切换）(它会对将generator函数转换为使用不污染全局作用的regenerator运行时，可以理解为做了一层兼容，因为 generator 是 es6 的语法)（所以当我们引入它之后项目可以自动支持 generators/async 语法。当然我们也可以用 generator 的插件来支持这种语法，如 [@babel/plugin-transform-runtime
